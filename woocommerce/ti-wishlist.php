...
?>
<div class="woocommerce-MyAccount-content"><div class="tinv-wishlist woocommerce tinv-wishlist-clear">
	...
	<form action="<?php echo esc_url($form_url); ?>" method="post" autocomplete="off">
		...
		<ul class="tinvwl-table-manage-list">
			<div class="remove"><thead>
			<tr>
				...
				if ($wl_product['quantity'] > 0 && apply_filters('tinvwl_wishlist_item_visible', true, $wl_product, $product)) {
					...
					<li class="<?php echo esc_attr(apply_filters('tinvwl_wishlist_item_class', 'wishlist_item', $wl_product, $product)); ?>">
						...
						<td class="product-remove">
							<button class="remove-wish" type="submit" name="tinvwl-remove"
									value="<?php echo esc_attr($wl_product['ID']); ?>"
									title="<?php _e('Remove', 'ti-woocommerce-wishlist') ?>"><i class="moon-icon icomoon-the7-font-the7-cross-00"></i>
							</button>
						</td>
						...
						<td class="product-name">
							...
							if (!$product->is_visible()) {
								...
								echo apply_filters('tinvwl_wishlist_item_name', sprintf('<div class="wishlist-prod-name"><a href="%s">%s</a></div>', esc_url($product_url), is_callable(array(
										$product,
										'get_name'
								)) ? $product->get_name() : $product->get_title()), $wl_product, $product); // WPCS: xss ok.
							}
							...
							$taxonomy = 'pa_style';
							 echo $product->get_categories( ', ', '<span class="wishlist-prod-category">' . _n( '', '', sizeof( get_the_terms( $post->ID, 'product_cat' ) ), 'woocommerce' ) . ' ', '</span> |' ); echo ' <span class="attribute-brand">' . $product->get_attribute($taxonomy) . '</span>';?>
						</td>
						<?php if (isset($wishlist_table_row['colm_price']) && $wishlist_table_row['colm_price']) { ?>
						...
						<?php } ?>
						<?php if (isset($wishlist_table_row['colm_date']) && $wishlist_table_row['colm_date']) { ?>
							<div class="remove"><td class="product-date">
								<?php
						...
						<?php } ?>
						<?php if (isset($wishlist_table_row['colm_stock']) && $wishlist_table_row['colm_stock']) { ?>
							<div class="remove"><td class="product-stock">
								<?php
